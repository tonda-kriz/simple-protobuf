...
{
    const auto enum_with_parent =
        std::string( parent ) + "::" + std::string( my_enum.name.get_name( ) );
...
{
    stream << "field_attributes{.name = \"" << json_field_name( field ) << "\"sv";
                                 ...
                                 std::string_view full_name )
{
    ...
    for( const auto & field : my_enum.fields )
    {
        name_map.emplace( spb::json::detail::djb2_hash( field.name.get_name( ) ),
                          field.name.get_name( ) );
                               ...
                               const proto_message & message, std::string_view full_name )
{
    ...
    for( const auto & map : message.maps )
    {
        ...
        stream << ", value." << map.name.get_name( ) << " );\n";
    }
                                 ...
                                 std::string_view full_name )
{
    ...
    for( const auto & field : message.maps )
    {
        ...
        if( field_name != field.name.proto_name )
        {
            ...
            name_map.emplace( spb::json::detail::djb2_hash( field.name.proto_name ),
                              one_field{ .parsed_name = std::string( field.name.proto_name ),
                                         .name        = field.name } );
        }
    }
    for( const auto & oneof : message.oneofs )
    {
        for( size_t i = 0; i < oneof.fields.size( ); ++i )
        {
            ...
            if( field_name != oneof.fields[ i ].name.proto_name )
            {
                ...
                name_map.emplace(
                    spb::json::detail::djb2_hash( oneof.fields[ i ].name.proto_name ),
                    one_field{
    ...
    for( const auto & [ hash, field ] : name_map )
    {
        ...
        if( field.oneof_index == SIZE_MAX )
        {
            if( !field.bitfield.empty( ) )
            {
                ...
                stream << "\t\t\t\tvalue." << field.name.get_name( )
                       << " = stream.deserialize_bitfield< decltype( value."
                       << field.name.get_name( ) << " ) >( " << field.bitfield
                       << " );\n\t\t\t\treturn ;\n";
            }
            else
            {

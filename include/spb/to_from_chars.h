...
static inline auto from_chars(const char *first, const char *last, std::integral auto &number, int base = 10)
    -> from_chars_result
{
    assert(first <= last);
    char buffer[32];
...
static inline auto from_chars(const char *first, const char *last, std::floating_point auto &number)
    -> from_chars_result
{
    assert(first <= last);
    char buffer[256];
